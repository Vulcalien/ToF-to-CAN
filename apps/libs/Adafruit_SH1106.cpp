...
Adafruit_SH1106::Adafruit_SH1106(FAR struct i2c_master_s *i2c_device)
    : Adafruit_GFX(SH1106_LCDWIDTH, SH1106_LCDHEIGHT),I2C_Dev(i2c_device, SH1106_I2C_ADDRESS)
{
}



bool Adafruit_SH1106::begin(uint8_t vccstate)
{
    _vccstate = vccstate;
...
void Adafruit_SH1106::SH1106_command(uint8_t c)
{
    ...
    if (!write_buf((uint8_t *)buf, 2)) {
        printf("Adafruit_SH1106: cannot write i2c in %s\n", __func__);
    }
}
...
uint8_t row_buffer[SH1106_LCDWIDTH + 1];
...
void Adafruit_SH1106::display(void)
{
    ...
    for ( i = 0; i < height; i++) {
        ...
        for( j = 0; j < 8; j++){
            ...
            memcpy(&row_buffer[1], &buffer[p], width);
            row_buffer[0] = 0x40;
            if (!write_buf((uint8_t *)row_buffer, width + 1)) {
                printf("Adafruit_SH1106: cannot write i2c in %s\n", __func__);
            }
            p += width;
