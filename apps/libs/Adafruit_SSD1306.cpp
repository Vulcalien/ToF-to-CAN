...
#define SSD1306_ADDRESS 0x3C // (or 0x78??)
...
*/
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, FAR struct i2c_master_s *i2c_device)
    : Adafruit_GFX(w, h), I2C_Dev(i2c_device, SSD1306_ADDRESS), buffer(NULL)
{
}
...
void Adafruit_SSD1306::ssd1306_command1(uint8_t c) {
    uint8_t buf[2] = { 0x00, // Co = 0, D/C = 0
                       c };
    if (!write_buf((uint8_t *)buf, 2)) {
        printf("Adafruit_SSD1306: cannot write i2c in %s\n", __func__);
    }
}
...
/*!
   ...
   transactions. This is a protected function, not exposed.
    ...
    @note
*/
uint8_t i2c_tempbuf[2048];
void Adafruit_SSD1306::ssd1306_commandList(const uint8_t *c, int n, uint8_t first) {
    i2c_tempbuf[0] = first;
    memcpy(&i2c_tempbuf[1], c, n);
    if (!write_buf((uint8_t *)i2c_tempbuf, n + 1))
        printf("Adafruit_SSD1306: cannot write i2c in %s\n", __func__);
  // if (wire) { // I2C
...
bool Adafruit_SSD1306::begin(uint8_t vcs) {
  ...
  drawBitmap((WIDTH - unict_team_width) / 2, (HEIGHT - unict_team_height) / 2,
             unict_team_data, unict_team_width, unict_team_height, 1);
