...
Pid::Pid(float kp, float ki, float kd, float kff, float saturation)
    : m_Kp(kp), m_Ki(ki), m_Kd(kd), m_Kff(kff), m_saturation(saturation)
...
Pid::Pid()
    : m_Kp(0), m_Ki(0), m_Kd(0), m_Kff(0), m_saturation(0)
{
...
void Pid::init(float kp, float ki, float kd, float kff, float saturation)
{
...
float Pid::evaluate(float target, float process_var, float delta_t)
{
    float error = target - process_var;
    return evaluate_using_error(error, target, process_var, delta_t);
}

float Pid::evaluate_using_error(float error, float target, float process_var, float delta_t)
{
    float u_p, u_d;
    ...
    if (m_Kd != 0.0)
        u_d  = m_Kd *  derivative_low_pass(derivative(error, process_var)) / delta_t;
    else
        u_d = 0.0;
    ...
    if (!m_windup)
        m_u_i += m_Ki * (((error + m_error_old) / 2.0) * delta_t);

    m_out = u_p + m_u_i + u_d + target * m_Kff;

    //printf("%f %f %f ", target, process_var, m_out);
