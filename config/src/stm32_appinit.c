...
int board_app_initialize(uintptr_t arg)
{
    ...
    ret = board_timer_driver_initialize("/dev/timer0", 16);
    if (ret != OK)
        {
    ...
    syslog(LOG_INFO, "Register the qencoder %s as TIM1\n", buf);
#endif
  ...
  if (ret < 0)
    ...
    stm32l4_configgpio(I2C_RST);
    stm32l4_configgpio(LPn);
    syslog(LOG_INFO, "CONFIGURATION COMPLETE\n");

    return ret;
}

void set_i2c_rst(bool on)
{
    stm32l4_gpiowrite(I2C_RST, on);
}


void set_LPn(bool on)
{
    stm32l4_gpiowrite(LPn, on);
}
